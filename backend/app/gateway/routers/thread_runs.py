...
async def _find_target_run_id(thread_id: str, message_id: str, request: Request) -> str:
    event_store = get_run_event_store(request)
    rows = await event_store.list_messages(thread_id, limit=200)
    ...
    fallback_record = next((record for record in records if record.status == RunStatus.success), None)
    fallback_record = fallback_record or (records[0] if records else None)
...
async def _find_base_checkpoint_before_human(thread_id: str, human_message_id: str, request: Request) -> Any:
    ...
    try:
        checkpoints = [item async for item in checkpointer.alist(base_config, limit=200)]
    ...
    for checkpoint_tuple in reversed(checkpoints):
        ...
        if human_message_id in message_ids:
            if previous_checkpoint is None:
                raise HTTPException(status_code=409, detail="Could not find checkpoint before target human message")
            return previous_checkpoint
        if _checkpoint_configurable(checkpoint_tuple).get("checkpoint_id"):
            previous_checkpoint = checkpoint_tuple

    raise HTTPException(status_code=409, detail="Could not locate target human message in checkpoint history")
...
async def _prepare_regenerate_payload(thread_id: str, message_id: str, request: Request) -> RegeneratePrepareResponse:
    ...
    target_run_id = await _find_target_run_id(thread_id, message_id, request)
