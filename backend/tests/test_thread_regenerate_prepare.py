...
def test_prepare_regenerate_payload_falls_back_to_latest_run_when_events_are_missing():
    from app.gateway.routers.thread_runs import _prepare_regenerate_payload
    ...
    run_manager = FakeRunManager(
        [
            SimpleNamespace(run_id="run-latest", status=RunStatus.success),
            SimpleNamespace(run_id="run-older", status=RunStatus.error),
        ]
    )
