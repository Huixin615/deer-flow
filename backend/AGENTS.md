...
head. Legacy single-checkpoint branches lazily copy that anchor from their recorded
parent thread when `POST /runs/regenerate/prepare` first needs it; the repair is
idempotent and fails with 409 if the source checkpoint no longer exists.
...
**Replay checkpoint lookup prefers lineage and degrades only for legacy data.** Branch and regenerate paths first walk `parent_config`, which prevents a global chronological scan from selecting a sibling created by regeneration. Only when those links are absent do they use the bounded newest-first history fallback in `app/gateway/checkpoint_lineage.py`; the fallback scans oldest-to-newest internally, skips duration-only checkpoints, and accepts only checkpoints with an addressable id as the replay base. A branch with no discoverable pre-user checkpoint must preserve the historical single-checkpoint branch behavior instead of rejecting the branch; regeneration remains unavailable for that inherited response. Storage or checkpoint-mode failures are not treated as a missing base and still fail closed.
