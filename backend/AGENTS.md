...
- Orphan reconciliation treats `list_inflight_with_expired_lease()` as candidate selection only. Every candidate must be finalized through `claim_for_takeover()` with the same grace period so the final conditional write re-checks active status and lease expiry; only successful claims may be returned to recovery callbacks.
