...
16. **SkillToolPolicyMiddleware** - Applies `allowed-tools` only to the current run's slash-activated skill and skills actually loaded into `ThreadState.skill_context`; passive enabled skills do not clamp the lead toolset. It filters model-visible schemas and blocks unauthorized execution, resolving canonical paths against the live enabled/agent-allowed registry on every call. `task` is not framework-exempt, so a restricted skill cannot delegate around its policy.
