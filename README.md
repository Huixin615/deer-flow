...
An enabled skill's `allowed-tools` policy applies only after that skill is explicitly slash-activated or captured in the thread's active skill context after a `read_file` load. Merely enabling or advertising a skill does not reduce the lead agent's normal toolset; once active, the policy filters both model-visible tool schemas and tool execution. Per-step policy decisions are internal runtime context and are removed from observable or persisted context copies. This is best-effort behavioral scoping, not a hard security boundary: loading skill instructions through another tool is not captured, active-skill entries can be evicted from bounded context, and stale paths for removed skills are ignored.
